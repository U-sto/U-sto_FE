...
const DisposalManagementPage = () => {
  ...
  const filteredRegistrationData = useMemo(() => {
    return allRegistrationData.filter((row) => {
      if (filters.disposalDateFrom && row.disposalDate < filters.disposalDateFrom) return false
      if (filters.disposalDateTo && row.disposalDate > filters.disposalDateTo) return false
